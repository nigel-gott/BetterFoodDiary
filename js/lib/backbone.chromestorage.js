(function(factory) {
  ...
  function ChromeStorage(name, type) {
    ...
    this.loaded = this.store.get(this.name).
      pipe(this._parseRecords).
        ...
        chrome.storage.onChanged.addListener(record_updater.bind(this));
      }).bind(this));
  }

  function record_updater(changes, type){
      if(our_records_have_changed(changes, type)){
        this.records = changes[this.name].newValue.split(',');
      }
  }
  ...
  function our_records_have_changed(changes, type){
      return 
          type === this.type && 
          changes[this.name] && 
          changes[this.name].newValue !== this.records.join(',');
  }
  ...
  _(ChromeStorage.prototype).extend({
    ...
    findAll: function() {
      ...
      $.when(this.loaded).done((function(records) {
        ...
        this.store.get(model_ids, function(result){
            resolveModels(result);
        });
      }).bind(this));
